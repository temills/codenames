#1/2022
#Tracey Mills, modified from Shuangyin Li
...
def inference(word, context, sentence_string, pwe):
    ...
    for keyword in select_keywords_set:
        ...
        = updatewordsemanticwithcontext(keyword, context, pwe, sentence_doc)
...
if __name__ == '__main__':
    ...
    for i in range(len(words)):
        ...
        wordvectors = inference(word, context, asentence_string, pwe)
        ...
        #get rating
        vecs1.append(wordvectors[0])
        vecs2.append(wordvectors[5])
    print(vecs2)
    print('Cos similiarity between ' + words[0] + ' and ' + words[1])
    print(np.dot(vecs1[0], vecs1[1])/(norm(vecs1[0])*norm(vecs1[1])))
    print([1 - spatial.distance.cosine(vecs1[0], vecs1[1]), 1 - spatial.distance.cosine(vecs2[0], vecs2[1])])
